...
def select_wav():
	...
	lines = read_files("manifest.train")
	wav_select = []
	wav_path_list = []
	wav_text_list = []
...
def statistic_info_2():
	...
	for one_line in tqdm(lines):
		try:
			one_line_json = eval(one_line)
			wav_dur = float(one_line_json["duration"])
			all_time = all_time + wav_dur
			if wav_dur > 0.5:
				lines_1.append(one_line)
				lines_1_time =lines_1_time+wav_dur
			if wav_dur < 0.5:
				lines_0.append(one_line)
				lines_0_time =lines_0_time+wav_dur
